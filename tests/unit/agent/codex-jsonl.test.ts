...
describe('Codex JSONL translator', () => {
  ...
  it('treats duplicate announcements as one final message', () => {
    const t = new CodexJsonlTranslator();
  ...
  it('flushes a pending message before an abnormal terminal error', () => {
    const t = new CodexJsonlTranslator();

    expect(t.translate({ type: 'agent_message', message: 'last visible progress' })).toEqual([]);
    expect(t.translate({ type: 'turn.failed', error: { message: 'model stopped' } })).toEqual([
      { type: 'text', delta: 'last visible progress' },
      { type: 'error', message: 'model stopped', terminationReason: 'failed' },
    ]);
  });

  it('flushes a pending message before an adapter runtime failure', () => {
    const t = new CodexJsonlTranslator();

    expect(t.translate({ type: 'agent_message', message: 'last visible progress' })).toEqual([]);
    expect(t.fail('codex exited with code 1')).toEqual([
      { type: 'text', delta: 'last visible progress' },
      { type: 'error', message: 'codex exited with code 1', terminationReason: 'failed' },
    ]);
  });
