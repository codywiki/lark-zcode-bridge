...
export async function runSecretsSet(
  ...
  if (!appId) {
    console.error('用法: lark-zcode-bridge secrets set --app-id <id>');
    process.exit(1);
  }
  ...
  console.log(`✓ 已加密存到 ~/.lark-zcode-bridge/secrets.enc`);
}
...
export async function runSecretsRemove(
  ...
  if (!appId) {
    console.error('用法: lark-zcode-bridge secrets remove --app-id <id>');
    process.exit(1);
  }
