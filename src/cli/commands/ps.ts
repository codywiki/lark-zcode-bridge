...
/**
 * Pretty-print the list of running lark-zcode-bridge processes.
 *
...
export async function runKillCli(target: string | undefined): Promise<void> {
  if (!target) {
    console.error('用法: lark-zcode-bridge kill <bot id 或序号>');
    process.exit(1);
  }
  const entry = resolveTarget(target);
  if (!entry) {
    console.error(`✗ 没找到匹配的 bot:${target}`);
    console.error('  用 `lark-zcode-bridge ps` 看可选目标。');
    process.exit(1);
  }
